...
public abstract class ElicomWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
{
    protected static readonly Lazy<string> ContentRootFolder;

    static ElicomWebTestBase()
    {
    ...
    {
        using (var context = IocManager.Resolve<ElicomDbContext>())
        {
            action(context);
            context.SaveChanges();
        }
    ...
    protected async Task UsingDbContextAsync(Func<ElicomDbContext, Task> action)
    {
        using (var context = IocManager.Resolve<ElicomDbContext>())
        {
    ...
    protected async Task<T> UsingDbContextAsync<T>(Func<ElicomDbContext, Task<T>> func)
    {
        T result;
